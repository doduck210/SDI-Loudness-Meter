(function () {
    ...
    function setupInputConfigPanel() {
        const toggleBtn = document.getElementById('inputConfigToggle');
        const panel = document.getElementById('inputConfigPanel');
        const deviceSelect = document.getElementById('inputConfigDevice');
        const videoSelect = document.getElementById('inputConfigVideo');
        const audioSelect = document.getElementById('inputConfigAudio');
        const applyBtn = document.getElementById('inputConfigApply');
        ...
        const loadOptions = async (deviceId) => {
            ...
            try {
                ...
                if (!res.ok) throw new Error(res.statusText);
                const data = await res.json();
                fillSelect(videoSelect, data.videoInputs);
                fillSelect(audioSelect, data.audioInputs);
                setStatus('옵션 로드 완료');
            } catch (err) {
                console.error(err);
        ...
        applyBtn.addEventListener('click', async () => {
            ...
            try {
                const payload = {
                    device: deviceSelect.value,
                    videoInputId: videoSelect.disabled ? undefined : videoSelect.value,
                    audioInputId: audioSelect.disabled ? undefined : audioSelect.value
                };
